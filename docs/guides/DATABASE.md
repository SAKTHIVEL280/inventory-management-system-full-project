...
- **Development**: SQLite (no setup) or PostgreSQL (Docker)
- **Production**: PostgreSQL 15+ (required)
- **ORM**: SQLAlchemy 2.x with UUID primary keys
- **Migrations**: Alembic (version control for schema)

---

## 🚀 Option 1: Quick Start (SQLite - Zero Setup)
...
### Setup
```bash
# The app auto-creates SQLite database
...
## 🐘 Option 2: Production Setup (PostgreSQL with Docker)
...
**What it does:**
1. ✅ Connects to database (PostgreSQL or SQLite)
...
2. Delete .db file: rm ims.db
3. Re-run: python setup_db.py
```
...
```bash
# Simple copy
cp ims.db ims.db.backup

# Restore
cp ims.db.backup ims.db
```
...
- [ ] Decide: SQLite (quick dev) or PostgreSQL (real setup)?
...
A: Not recommended. The codebase uses PostgreSQL-specific features (UUID type, JSON). SQLite alternative provided for development.
...
**Database Version:** PostgreSQL 15+ / SQLite 3
