...
## BE-133: Partial vs full payment status display
**Update**: Added receipt/payment display derivation so customer receipts show `Partially Received` vs `Fully Received`, and supplier payments show `Partially Paid` when cleared but not fully settled.
