...
const SalesOrdersPage = () => {
  ...
  const fetchOrders = async () => {
    try {
      setLoading(true);
      const res = await salesApi.listSalesOrders(statusFilter || undefined, 1, 20, {
  ...
  const filteredOrders = orders.filter((o) => {
    ...
    const customerName = customerNameById(o.customer_id).toLowerCase();
    const matchesSearch =
      !q ||
  ...
  const handleArchiveToggle = async (id: string, archived: boolean) => {
    try {
      if (archived) {
        await salesApi.restoreSalesOrder(id);
      } else {
    ...
    <AppLayout title="Sales Orders">
      <div className="space-y-6">
        ...
        <div className="hms-card overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                : filteredOrders.length === 0 ? <tr><td colSpan={6} className="px-4 py-8 text-center text-neutral-500">No sales orders</td></tr>
                : filteredOrders.map(o => (
                  <tr key={o.id} className="border-b border-neutral-100 hover:bg-neutral-50">
                    ...
                    <td className="px-4 py-3">{customerNameById(o.customer_id)}</td>
          ...
          {!loading && <p className="border-t border-neutral-200 px-4 py-3 text-xs text-neutral-500">Showing {filteredOrders.length} of {orders.length}</p>}
        </div>
